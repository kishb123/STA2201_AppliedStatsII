...
First, we plot the likelihood $L(y;\theta) = {129 \choose 118}\ \theta^{118}(1-\theta)^{11}$
...
ggplot(data = df2, aes(x = theta)) +
  ...
  stat_function(fun = dbeta, args = c(y + 10,n - y - 10),
                ...
                fill = "red", alpha = 0.5) 
```
Again, red reflects the Beta(10,10) prior, and blue represents the Beta(1,1) prior. So when we have a prior of Beta(10,10), our distribution has lots of density near $1$. However, with a `uninformative' prior, our posterior is less sure, as we did not include as much prior knowledge. 
...
Players are more likely to get better than worse, so positive values of theta are more likely, so I could do a Normal(0.4, 0.2) prior (note that this makes values that are impossible such as theta > 1 or theta < -1 occur with very small probability).
