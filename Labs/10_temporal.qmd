...
```
We see that when there is less data the trend loses much of its characteristic linear property. Now it seems as though child mortality might be decreasing, after it increased. This means that our prediction learns this, and the resulting output child mortality is smaller than it was in previous years. This is a good thing, but does not follow the general trend we have seen, so it is a little more unlike the data. 
